...
export const MessagesPage: React.FC = () => {
  ...
  const eventsTab = {
      name: "Events",
      number: 0,
      component: <h2 className="text-center text-[16px] font-[500] leading-[1.2] font-poppins text-[#959494]">No Events</h2> }
  ...
  const fetchLocations = () => {
      getLocationList(tokenString).then((res) => {
      setLocations(res);
    }).catch((err: any) => {
  ...
  const fetchMessages = () => {
      getAllMsgStream(tokenString).then((res) => {
      setMessages(res);
    }).catch((err: any) => {
     setError(err.message);
    });
