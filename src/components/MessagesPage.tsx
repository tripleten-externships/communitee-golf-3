...
import { getLocations, getAllMsgStream } from "../services/api";
...
export const MessagesPage: React.FC = () => {
  ...
  const eventsTab = {
      name: "Events",
      number: 0,
      component: <h2 className="text-center text-[16px] font-[500] leading-[1.2] font-poppins text-[#959494]">Events Component</h2> }
  ...
  const fetchMessages = () => {
      getAllMsgStream(token).then((res) => {
      setMessages(res);
    }).catch((err: any) => {
     setError(err.message);
    });
